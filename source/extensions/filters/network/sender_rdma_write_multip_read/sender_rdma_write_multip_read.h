 ...
 */
class SenderRDMAWriteMultipReadFilter : 
                     public Network::ReadFilter,
                     public Network::WriteFilter,
                     public Network::ConnectionCallbacks,
                     public std::enable_shared_from_this<SenderRDMAWriteMultipReadFilter>,
...
public:
    ...
    // Destructor
    ~SenderRDMAWriteMultipReadFilter() {
    ...
    // Constructor
    SenderRDMAWriteMultipReadFilter(const uint32_t payloadBound, const uint32_t circleSize, const uint32_t timeToWrite, const uint32_t sharedBufferSize)
    ...
    int can_write(uint32_t offset, volatile uint32_t *limit) {	
	    if (~(*limit)!=*remoteChksum_) return 0;
        return offset!=*limit;
    }
    ...
    void setup_rdma() {
        ...
        *hostChksum_ = ~(*hostLimit_);
        ...
        rdma_sender_thread_ = std::thread(&SenderRDMAWriteMultipReadFilter::rdma_sender, this);
        rdma_polling_thread_ = std::thread(&SenderRDMAWriteMultipReadFilter::rdma_polling, this);
        downstream_sender_thread_ = std::thread(&SenderRDMAWriteMultipReadFilter::downstream_sender, this);
    }
    ...
    void rdma_sender() {
        ...
        while (true) {
            ...
            if (downstream_to_upstream_buffer_->pop(item)) { // to opti                
                ...
                while (!can_write(offset, remoteLimit_)) {
                    qpToWrite_->read(remoteMemory_, remoteMemoryToken_, 2*sizeof(uint32_t), &requestTokenRead);
                    requestTokenRead.waitUntilCompleted();
