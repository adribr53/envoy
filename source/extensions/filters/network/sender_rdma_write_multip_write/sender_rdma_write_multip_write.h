 ...
 */
class SenderRDMAWriteMultipWriteFilter : 
                     public Network::ReadFilter,
                     public Network::WriteFilter,
                     public Network::ConnectionCallbacks,
                     public std::enable_shared_from_this<SenderRDMAWriteMultipWriteFilter>,
...
public:
    ...
    // Destructor
    ~SenderRDMAWriteMultipWriteFilter() {
    ...
    // Constructor
    SenderRDMAWriteMultipWriteFilter(const uint32_t payloadBound, const uint32_t circleSize, const uint32_t timeToWrite, const uint32_t sharedBufferSize)
    ...
    void setup_rdma() {
        ...
        *hostChksum_ = ~(*hostLimit_);
        ...
        rdma_sender_thread_ = std::thread(&SenderRDMAWriteMultipWriteFilter::rdma_sender, this);
        rdma_polling_thread_ = std::thread(&SenderRDMAWriteMultipWriteFilter::rdma_polling, this);
        downstream_sender_thread_ = std::thread(&SenderRDMAWriteMultipWriteFilter::downstream_sender, this);
    }
    ...
    void rdma_sender() {
        ...
        while (true) {
            ...
            if (downstream_to_upstream_buffer_->pop(item)) { // to opti                
                ...
                while (!can_write(offset, hostLimit_)) {
