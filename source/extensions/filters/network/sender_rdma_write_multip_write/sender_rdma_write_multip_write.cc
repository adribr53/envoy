...
Network::FilterStatus SenderRDMAWriteMultipWriteFilter::onNewConnection() {
    ENVOY_LOG(debug, "onNewConnection triggered");
    return Network::FilterStatus::Continue;
}

// Event called when receiving data from the client (through the listener)
Network::FilterStatus SenderRDMAWriteMultipWriteFilter::onData(Buffer::Instance& data, bool end_stream) {
    ENVOY_CONN_LOG(trace, "sender_rdma_write_multip_write: got {} bytes", read_callbacks_->connection(), data.length());
...
Network::FilterStatus SenderRDMAWriteMultipWriteFilter::onWrite(Buffer::Instance& data, bool end_stream) {
    ENVOY_CONN_LOG(trace, "sender_rdma_write_multip_write: got {} bytes", write_callbacks_->connection(), data.length());
