...
Network::FilterStatus SenderFilter::onNewConnection() {
  ...
  char ack[6];
  memset(ack, '\0', 6);
  if (recv(sock_, ack, 6, 0) < 0) { // Should not block on recv in case of failure of upstream listener
    ENVOY_LOG(error, "Error in receiving ack");
    close(sock_);
    close(sock_rdma_);
    read_callbacks_->connection().close(Network::ConnectionCloseType::Abort);
    return Network::FilterStatus::StopIteration;
...
Network::FilterStatus SenderFilter::onData(Buffer::Instance& data, bool end_stream) {
  ...
  ENVOY_LOG(debug, "AFTER SOCK DISTANT RDMA SEND");
